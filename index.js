...
// Only Guilds + GuildMessages for mention replies (MessageContent NOT needed to detect mentions)
...
let mentionCooldown = { count: 0, timestamp: Date.now() };
...
// Mention reply handler (no MessageContent intent needed — mentions still fire)
client.on('messageCreate', async message => {
    if (message.author.bot) return;
    if (!message.mentions.has(client.user)) return;

    const now = Date.now();
    if (now - mentionCooldown.timestamp > 60 * 1000) {
        mentionCooldown.count = 0;
        mentionCooldown.timestamp = now;
    }
